...
public class WarningCopy extends PageHandler<SourceWarning> {
    ...
    protected int handleDataOfPerPage(List<SourceWarning> list, int pageNumber) {
        int size = list.size();
        log.info("开始请求es获取数据..");
        List<String> articleIdList = list.stream().map(SourceWarning::getId).collect(Collectors.toList());
        ...
        for (int i = 0; i < size; i++) {
            SourceWarning sourceWarning = list.get(i);
            Long sourceWarningDeleteState = sourceWarning.getDeleteState();

            if (sourceWarningDeleteState == 0) {
...
//
                    ...
                    log.info("id为{}文章es中不存在，不进行添加");
                }
            } else {
                log.info("id为{}文章以删除，不进行添加");
            }
            cpb.show(i);
        }
        return size;
